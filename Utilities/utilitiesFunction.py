...
import talib
import math
import Utilities.sendEmail as sendEmail
import pprint
import requests
...
def updateDatabase(list_tickers = [], interval = INTERVAL1+INTERVAL2):
	if len(list_tickers) >0 :
		retrieveDataUpdating(list_tickers,interval)
	else:
...
def retrieveDataUpdating(list_tickers,interval):
...
def retrieveData(list_tickers,howLong=30, interval = INTERVAL1+INTERVAL2,end_date = datetime.datetime.now(),writeOnFileTrigger = False):
