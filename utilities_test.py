...
def find_minimum(df):
	...
	for item in np.array_split(df,55):
		if len(item) ==0:
			continue
		minimum.append(item['close'].min())
		index_minimum.append(item['close'].idxmin())  #if you find a minimum alonf the index column use idxmin()
...
def build_DataFrame_to_write_on_file(df_on_file,df_minimum):
	...
	df_on_file = compute_differences(df_on_file)
